...
def add_spend_range_features(
    ...
    # Find rows where gamma_eom is missing
    missing_rows = x[x["gamma_eom"].isna()]

    if not missing_rows.empty:
        # Collect game_ids and the months where gamma_eom is missing
        missing_info = (
            missing_rows
            .groupby("game_id")["month"]
            .unique()
            .to_dict()
        )
        print("Missing gamma_eom detected:")
        for game_id, months in missing_info.items():
            print(f"game_id {game_id}: months {list(months)}")

        # Drop rows (months) where gamma_eom is missing
        x = x.dropna(subset=["gamma_eom"])
