...
CLIENT_DRIVER_WHITELIST = [
    # gap / pacing
    "gamma_gap_mtd", "expected_rev_needed", "gap_per_remaining_day", "late_gap",
    # revenue curve
    ...
    # optional summary proxy (keep if you can explain it)
    "cohort_roas_proxy_d7",
]

# Friendly labels + units + formatting.
# You can tweak these labels without touching the logic.
FEATURE_META = {
    "gamma_gap_mtd": dict(label="Gap to EOM ROAS target (MTD)", unit="ROAS", fmt="{:+.3f}"),
    "expected_rev_needed": dict(label="Revenue still needed to hit target", unit="$", fmt="{:,.0f}"),
    "gap_per_remaining_day": dict(label="Required revenue per remaining day", unit="$/day", fmt="{:,.0f}"),
